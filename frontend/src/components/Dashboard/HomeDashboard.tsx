...
export default function HomeDashboard() {
  ...
  const handleSelectDoc = (doc: PDFDocument, tab: 'pages' | 'summary' | 'ai' | 'compare' = 'ai') => {
  ...
  const triggerBackendAnalysis = async (docId: string, file: File) => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/ai/contracts/${docId}/analyze`, {
          method: 'POST',
