...
export default function LeftSidebar() {
  ...
  const publishDocumentToQr = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3001/documents/publish', {
        method: 'POST',
        body: formData
      });
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        const res = await fetch(`http://localhost:3001/qr/stats/${qrConfig.qrToken}`);
        if (!res.ok) return;
        ...
        <div className="relative z-10 h-full flex flex-col">
          ...
          {activeTab === 'summary' && currentDocument && (
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center text-gray-500">
              <FileText size={48} className="mb-4 text-purple-500/30" />
              <p className="text-sm font-medium">Executive Summary & Risk Dashboard is active in the main viewport.</p>
            </div>
          )}
