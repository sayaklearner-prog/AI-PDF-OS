...
export const ClauseExplorer = ({ documentId }: { documentId: string }) => {
  ...
  const fetchClauses = async () => {
    try {
      const res = await axios.get(`http://localhost:3001/ai/contracts/${documentId}/clauses`);
      setClauses(res.data);
    } catch (e) {
  ...
  const handleRewriteAction = async (clauseId: string, action: 'accept' | 'reject') => {
    try {
      await axios.post(`http://localhost:3001/ai/contracts/clauses/${clauseId}/${action}-rewrite`);
      // Optimistic update
